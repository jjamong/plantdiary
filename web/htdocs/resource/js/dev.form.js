...
(function($) {
	...
	// inspector 내에서 input에 텍스트를 띄우는 함수
	$_FormInputMsg = function(input,text) {
		...
		userPw : function(form,params) {
			...
			if(!!option.compare) {
				if(input.value != form[option.compare].value) {
					if (params.option.msgType) {
						$_FormInputMsg(form[option.compare],params.text + $Util.getAux(params.text,'이') +' 동일하지 않습니다.');
